...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 QNO at the same time
    ONLY_MNCOLLATERAL = 5                        // find masternode outputs including locked ones (use with caution)
};
...
public:
    ...
    std::string PreparePrivatesendDenominate(int minRounds, int maxRounds);
    int GeneratePrivatesendOutputs(int nTotalValue, std::vector<CTxOut>& vout);
