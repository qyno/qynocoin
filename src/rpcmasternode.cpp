...
Value privatesend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "privatesend <qynoaddress> <amount>\n"
            "qynoaddress, reset, or auto (AutoDenominate)"
    ...
    if (params[0].get_str() == "auto") {
        if (fMasterNode)
            return "PrivateSend is not supported from masternodes";

        return "DoAutomaticDenominating " + (privateSendPool.DoAutomaticDenominating() ? "successful" : ("failed: " + privateSendPool.GetStatus()));
    }
    ...
    if (params.size() != 2)
        throw runtime_error(
            "privatesend <qynoaddress> <amount>\n"
            "qynoaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) Qyno address of current masternode\n"
    ...
    obj.push_back(Pair("entries_accepted", privateSendPool.GetCountEntriesAccepted()));
    return obj;
}
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in qyno.conf\n"
...
Value listmasternodes(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",    (string) Masternode Qyno address\n"
...
Value masternodeconnect(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:39741\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:39741\""));
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) Qyno address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
Value getmasternodewinners (const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) Qyno MN Address\n"
