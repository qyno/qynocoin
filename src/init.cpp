...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "qyno.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "qynod.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 39741, 39743));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in QNO/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in QNO/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, qyno, (privatesend, instantsend, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in QNO/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Qyno specific functionality (Masternodes, Privatesend, InstantSend, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:39741"));
    ...
    strUsage += HelpMessageOpt("-privatesendrounds=<n>", strprintf(_("Use N separate masternodes to anonymize funds  (2-8, default: %u)"), 2));
    strUsage += HelpMessageOpt("-anonymizeqynoamount=<n>", strprintf(_("Keep N QNO anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-enableinstantsend=<n>", strprintf(_("Enable instantsend, show confirmations for locked transactions (bool, default: %s)"), "true"));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 39742, 39744));
...
std::string LicenseInfo()
{
    return FormatParagraph(_("Copyright (C) 2009-2014 The Bitcoin Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(_("Copyright (C) 2014-2015 The Dash Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(_("Copyright (C) 2015-2017 The PIVX Developers")) + "\n" +
           "\n" +		
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Qyno Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Qyno is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Qyno Core is shutting down."));
...
#endif
    // Make sure only a single Qyno process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Qyno Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Qyno Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Qyno Core to complete") << "\n";
    ...
    if (fMasterNode) {
        LogPrintf("IS PRIVATESEND MASTER NODE\n");
        ...
        if (!strMasterNodePrivKey.empty()) {
            ...
            if (!privateSendSigner.SetKey(strMasterNodePrivKey, errorMessage, key, pubkey)) {
    ...
    fEnablePrivatesend = GetBoolArg("-enableprivatesend", false);

    nPrivatesendRounds = GetArg("-privatesendrounds", 2);
    if (nPrivatesendRounds > 16) nPrivatesendRounds = 16;
    if (nPrivatesendRounds < 1) nPrivatesendRounds = 1;

    nLiquidityProvider = GetArg("-liquidityprovider", 0); //0-100
    if (nLiquidityProvider != 0) {
        privateSendPool.SetMinBlockSpacing(std::min(nLiquidityProvider, 100) * 15);
        fEnablePrivatesend = true;
        nPrivatesendRounds = 99999;
    }

    nAnonymizeQynoAmount = GetArg("-anonymizeqynoamount", 0);
    if (nAnonymizeQynoAmount > 999999) nAnonymizeQynoAmount = 999999;
    if (nAnonymizeQynoAmount < 2) nAnonymizeQynoAmount = 2;
    ...
    LogPrintf("Anonymize Qyno Amount %d\n", nAnonymizeQynoAmount);
    LogPrintf("Budget Mode %s\n", strBudgetMode.c_str());

    /* Denominations

       A note about convertability. Within Privatesend pools, each denomination
       is convertable to another.

       For example:
       1QNO+1000 == (.1QNO+100)*10
       10QNO+10000 == (1QNO+1000)*10
    */
    privateSendDenominations.push_back((10000 * COIN) + 10000000);
    privateSendDenominations.push_back((1000 * COIN) + 1000000);
    privateSendDenominations.push_back((100 * COIN) + 100000);
    privateSendDenominations.push_back((10 * COIN) + 10000);
    privateSendDenominations.push_back((1 * COIN) + 1000);
    privateSendDenominations.push_back((.1 * COIN) + 100);
    /* Disabled till we need them
    privateSendDenominations.push_back( (.01      * COIN)+10 );
    privateSendDenominations.push_back( (.001     * COIN)+1 );
    */

    privateSendPool.InitCollateralAddress();
