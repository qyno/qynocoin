...
/** 
 * Fees smaller than this (in quffs) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_MNCOLLATERAL);
...
{
    BOOST_FOREACH (CAmount d, privateSendDenominations)
        if (nInputAmount == d)
            return true;
    return false;
}
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                    (!IsLockedCoin((*it).first, i) || nCoinType == ONLY_MNCOLLATERAL) &&
...
bool less_then_denom(const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH (CAmount d, privateSendDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 QNO
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, privateSendDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 QNO over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputPrivatesendRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nPrivatesendRoundsMin, int nPrivatesendRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 QNO+1 ( bit on if present )
            // bit 1 - 1000 QNO+1
            // bit 2 - 100 QNO+1
            // bit 3 - 10 QNO+1
            // bit 4 - 1 QNO+1
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nPrivatesendRoundsMin, int nPrivatesendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nPrivatesendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
    ...
    */
    ...
    BOOST_FOREACH (CTxIn v, txCollateral.vin) {
        if (!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CPrivatesendPool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 QNO.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Privatesend non-denominated funds for this transaction that are not equal 10000 QNO.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-qyno-address
                    CScript scriptChange;
...
string CWallet::PreparePrivatesendDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
        return _("Error: Wallet locked, unable to create transaction!");

    if (privateSendPool.GetState() != POOL_STATUS_ERROR && privateSendPool.GetState() != POOL_STATUS_SUCCESS)
        if (privateSendPool.GetEntriesCount() > 0)
    ...
    */
    if (minRounds >= 0) {
        if (!SelectCoinsByDenominations(privateSendPool.sessionDenom, 0.1 * COIN, PRIVATESEND_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
            return _("Error: Can't select current denominated inputs");
    }

    LogPrintf("PreparePrivatesendDenominate - preparing privatesend denominate . Got: %d \n", nValueIn);
    ...
    */
    ...
    while (nStep < nStepsMax) {
        BOOST_FOREACH (CAmount v, privateSendDenominations) {
            ...
            bool fAccepted = false;
            if ((privateSendPool.sessionDenom & (1 << 0)) && v == ((10000 * COIN) + 10000000)) {
                fAccepted = true;
            } else if ((privateSendPool.sessionDenom & (1 << 1)) && v == ((1000 * COIN) + 1000000)) {
                fAccepted = true;
            } else if ((privateSendPool.sessionDenom & (1 << 2)) && v == ((100 * COIN) + 100000)) {
                fAccepted = true;
            } else if ((privateSendPool.sessionDenom & (1 << 3)) && v == ((10 * COIN) + 10000)) {
                fAccepted = true;
            } else if ((privateSendPool.sessionDenom & (1 << 4)) && v == ((1 * COIN) + 1000)) {
                fAccepted = true;
            } else if ((privateSendPool.sessionDenom & (1 << 5)) && v == ((.1 * COIN) + 100)) {
                fAccepted = true;
            }
            if (!fAccepted) continue;
    ...
    if (privateSendPool.GetDenominations(vOut) != privateSendPool.sessionDenom) {
    ...
    privateSendPool.SendPrivatesendDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
...
int CMerkleTx::GetTransactionLockSignatures() const
{
    ...
    if (!IsSporkActive(SPORK_2_INSTANTSEND)) return -3;
