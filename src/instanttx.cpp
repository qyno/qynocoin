...
void ProcessMessageInstantSend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (!IsSporkActive(SPORK_2_INSTANTSEND)) return;
    if (!masternodeSync.IsBlockchainSynced()) return;

    if (strCommand == "ix") {
        //LogPrintf("ProcessMessageInstantSend::ix\n");
        ...
        BOOST_FOREACH (const CTxOut o, tx.vout) {
            ...
            if (!o.scriptPubKey.IsNormalPaymentScript() && !o.scriptPubKey.IsUnspendable()) {
                LogPrintf("ProcessMessageInstantSend::ix - Invalid Script %s\n", tx.ToString().c_str());
                return;
            }
        }
        ...
        if (fAccepted) {
            ...
            LogPrintf("ProcessMessageInstantSend::ix - Transaction Lock Request: %s %s : accepted %s\n",
            ...
            LogPrintf("ProcessMessageInstantSend::ix - Transaction Lock Request: %s %s : rejected %s\n",
                pfrom->addr.ToString().c_str(), pfrom->cleanSubVer.c_str(),
                tx.GetHash().ToString().c_str());
            ...
            if (i != mapTxLocks.end()) {
                ...
                if ((*i).second.CountSignatures() >= INSTANTSEND_SIGNATURES_REQUIRED) {
                    if (!CheckForConflictingLocks(tx)) {
                        LogPrintf("ProcessMessageInstantSend::ix - Found Existing Complete IX Lock\n");
    ...
    } else if (strCommand == "txlvote") //InstantSend Lock Consensus Votes
    {
        CConsensusVote ctx;
        vRecv >> ctx;
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                ...
                if (mapUnknownVotes[ctx.vinMasternode.prevout.hash] > GetTime() &&
                    ...
                    LogPrintf("ProcessMessageInstantSend::ix - masternode is spamming transaction votes: %s %s\n",
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    if (n == -1) {
        LogPrint("instantsend", "InstantSend::DoConsensusVote - Unknown Masternode\n");
        return;
    }

    if (n > INSTANTSEND_SIGNATURES_TOTAL) {
        LogPrint("instantsend", "InstantSend::DoConsensusVote - Masternode not in the top %d (%d)\n", INSTANTSEND_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    LogPrint("instantsend", "InstantSend::DoConsensusVote - In the top %d (%d)\n", INSTANTSEND_SIGNATURES_TOTAL, n);
    ...
    if (!ctx.Sign()) {
        LogPrintf("InstantSend::DoConsensusVote - Failed to sign consensus vote\n");
        return;
    }
    if (!ctx.SignatureValid()) {
        LogPrintf("InstantSend::DoConsensusVote - Signature invalid\n");
        return;
    }
...
{
    int n = mnodeman.GetMasternodeRank(ctx.vinMasternode, ctx.nBlockHeight, MIN_INSTANTSEND_PROTO_VERSION);

    CMasternode* pmn = mnodeman.Find(ctx.vinMasternode);
    if (pmn != NULL)
        LogPrint("instantsend", "InstantSend::ProcessConsensusVote - Masternode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "InstantSend::ProcessConsensusVote - Unknown Masternode\n");
        mnodeman.AskForMN(pnode, ctx.vinMasternode);
        return false;
    }

    if (n > INSTANTSEND_SIGNATURES_TOTAL) {
        LogPrint("instantsend", "InstantSend::ProcessConsensusVote - Masternode not in the top %d (%d) - %s\n", INSTANTSEND_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("InstantSend::ProcessConsensusVote - Signature invalid\n");
    ...
    if (!mapTxLocks.count(ctx.txHash)) {
        LogPrintf("InstantSend::ProcessConsensusVote - New Transaction Lock %s !\n", ctx.txHash.ToString().c_str());
        ...
        LogPrint("instantsend", "InstantSend::ProcessConsensusVote - Transaction Lock Exists %s !\n", ctx.txHash.ToString().c_str());
    ...
    if (i != mapTxLocks.end()) {
        ...
        LogPrint("instantsend", "InstantSend::ProcessConsensusVote - Transaction Lock Votes %d - %s !\n", (*i).second.CountSignatures(), ctx.GetHash().ToString().c_str());

        if ((*i).second.CountSignatures() >= INSTANTSEND_SIGNATURES_REQUIRED) {
            LogPrint("instantsend", "InstantSend::ProcessConsensusVote - Transaction Lock Is Complete %s !\n", (*i).second.GetHash().ToString().c_str());
    ...
    BOOST_FOREACH (const CTxIn& in, tx.vin) {
        if (mapLockedInputs.count(in.prevout)) {
            if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                LogPrintf("InstantSend::CheckForConflictingLocks - found two complete conflicting locks - removing both. %s %s", tx.GetHash().ToString().c_str(), mapLockedInputs[in.prevout].ToString().c_str());
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("InstantSend::CConsensusVote::SignatureValid() - Unknown Masternode\n");
        return false;
    }

    if (!privateSendSigner.VerifyMessage(pmn->pubKeyMasternode, vchMasterNodeSignature, strMessage, errorMessage)) {
        LogPrintf("InstantSend::CConsensusVote::SignatureValid() - Verify message failed\n");
        return false;
    }
...
bool CConsensusVote::Sign()
{
    ...
    if (!privateSendSigner.SignMessage(strMessage, errorMessage, vchMasterNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if (!privateSendSigner.VerifyMessage(pubkey2, vchMasterNodeSignature, strMessage, errorMessage)) {
