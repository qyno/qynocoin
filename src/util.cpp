...
const char * const BITCOIN_CONF_FILENAME = "qyno.conf";
const char * const BITCOIN_PID_FILENAME = "qynod.pid";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qyno";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Qyno";
#else
    // Unix
    return pathRet / ".qyno";
#endif
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty qyno.conf if it does not excist
