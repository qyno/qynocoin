                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    bool usePrivateSend = settings.value("bUsePrivateSend").toBool();
    bool useInstantSend = settings.value("bUseInstantSend").toBool();
    if (fLiteMode) {
        ui->checkUsePrivatesend->setChecked(false);
        ui->checkUsePrivatesend->setVisible(false);
        ui->checkInstantSend->setVisible(false);
        CoinControlDialog::coinControl->usePrivateSend = false;
        CoinControlDialog::coinControl->useInstantSend = false;
    } else {
        ui->checkUsePrivatesend->setChecked(usePrivateSend);
        ui->checkInstantSend->setChecked(useInstantSend);
        CoinControlDialog::coinControl->usePrivateSend = usePrivateSend;
        CoinControlDialog::coinControl->useInstantSend = useInstantSend;
    }

    connect(ui->checkUsePrivatesend, SIGNAL(stateChanged(int)), this, SLOT(updateDisplayUnit()));
    connect(ui->checkInstantSend, SIGNAL(stateChanged(int)), this, SLOT(updateInstantSend()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkInstantSend->isChecked()) {
        recipients[0].useInstantSend = true;
        strFunds += " ";
        strFunds += tr("and InstantSend");
    } else {
        recipients[0].useInstantSend = false;
    }
...
{
    QSettings settings;
    settings.setValue("bUseInstantSend", ui->checkInstantSend->isChecked());
    CoinControlDialog::coinControl->useInstantSend = ui->checkInstantSend->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Qyno address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    ui->checkUsePrivatesend->setChecked(CoinControlDialog::coinControl->usePrivateSend);
