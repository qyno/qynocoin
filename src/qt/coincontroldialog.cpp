...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            if (coinControl->usePrivateSend && rounds < nPrivatesendRounds) {
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (coinControl->useInstantSend) nPayFee = max(nPayFee, CENT);
        // Allow free?
        ...
        if (nPayAmount > 0) {
            ...
            // DS Fee = overpay
            if (coinControl->usePrivateSend && nChange > 0) {
                nPayFee += nChange;
                nChange = 0;
            }
    ...
    int nDisplayUnit = BitcoinUnits::QNO;
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        BOOST_FOREACH (const COutput& out, coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show Qyno address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            if (rounds >= 0)
                itemOutput->setText(COLUMN_PRIVATESEND_ROUNDS, strPad(QString::number(rounds), 11, " "));
            else
                itemOutput->setText(COLUMN_PRIVATESEND_ROUNDS, strPad(QString(tr("n/a")), 11, " "));
