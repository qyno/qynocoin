...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::QNO)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("QNO")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivatesendProgress()
{
    ...
    QString strAnonymizeQynoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeQynoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeQynoAmount = strAnonymizeQynoAmount.remove(strAnonymizeQynoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeQynoAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeQynoAmount * COIN) nMaxToAnonymize = nAnonymizeQynoAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeQynoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeQynoAmount));
        strAnonymizeQynoAmount = strAnonymizeQynoAmount.remove(strAnonymizeQynoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeQynoAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    } else {
...
void OverviewPage::privateSendStatus()
{
    ...
    if (((nBestHeight - privateSendPool.cachedNumBlocks) / (GetTimeMillis() - nLastDSProgressBlockTime + 1) > 1)) return;
    nLastDSProgressBlockTime = GetTimeMillis();

    if (!fEnablePrivatesend) {
        if (nBestHeight != privateSendPool.cachedNumBlocks) {
            privateSendPool.cachedNumBlocks = nBestHeight;
            updatePrivatesendProgress();
    ...
    if (privateSendPool.sessionDenom == 0) {
        ui->labelSubmittedDenom->setText(tr("N/A"));
    } else {
        std::string out;
        privateSendPool.GetDenominationsToString(privateSendPool.sessionDenom, out);
    ...
    if (!fEnablePrivatesend) {
        ...
        if (walletModel->getEncryptionStatus() == WalletModel::Locked) {
            ...
            if (!ctx.isValid()) {
                ...
                privateSendPool.cachedNumBlocks = std::numeric_limits<int>::max();
                QMessageBox::warning(this, tr("Privatesend"),
