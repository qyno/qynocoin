...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::QNO);
    ...
    if (!settings.contains("nAnonymizeQynoAmount"))
        settings.setValue("nAnonymizeQynoAmount", 1000);

    nPrivatesendRounds = settings.value("nPrivatesendRounds").toLongLong();
    nAnonymizeQynoAmount = settings.value("nAnonymizeQynoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeQynoAmount"))
        SoftSetArg("-anonymizeqynoamount", settings.value("nAnonymizeQynoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in qyno.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeQynoAmount:
            return QVariant(nAnonymizeQynoAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case PrivatesendRounds:
            ...
            break;
        case AnonymizeQynoAmount:
            nAnonymizeQynoAmount = value.toInt();
            settings.setValue("nAnonymizeQynoAmount", nAnonymizeQynoAmount);
            emit anonymizeQynoAmountChanged(nAnonymizeQynoAmount);
            break;
        case CoinControlFeatures:
